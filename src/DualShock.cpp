...
byte DualShock::configure(DualShockConfig dualshock_config) {
    this->dualshock_config = dualshock_config;
    clk_mask = (port_mask_t) digitalPinToBitMask(this->dualshock_config.pin_clk);
    clk_oreg = (port_reg_t*) portOutputRegister(digitalPinToPort(this->dualshock_config.pin_clk));
    cmd_mask = (port_mask_t) digitalPinToBitMask(this->dualshock_config.pin_cmd);
    cmd_oreg = (port_reg_t*) portOutputRegister(digitalPinToPort(this->dualshock_config.pin_cmd));
    att_mask = (port_mask_t) digitalPinToBitMask(this->dualshock_config.pin_att);
    att_oreg = (port_reg_t*) portOutputRegister(digitalPinToPort(this->dualshock_config.pin_att));
    dat_mask = (port_mask_t) digitalPinToBitMask(this->dualshock_config.pin_dat);
    dat_ireg = (port_reg_t*) portInputRegister(digitalPinToPort(this->dualshock_config.pin_dat));

    pinMode(this->dualshock_config.pin_clk, OUTPUT); //configure ports
    pinMode(this->dualshock_config.pin_att, OUTPUT); ATT_SET();
    pinMode(this->dualshock_config.pin_cmd, OUTPUT);
    pinMode(this->dualshock_config.pin_dat, INPUT_PULLUP); // enable pull-up

    // CMD_SET(); // SET(*_cmd_oreg,_cmd_mask);
    CLK_SET();

    return configureStub();
}
